(() => {
  ...
  var package_default = {
    ...
    peerDependencies: {
      leaflet: "github:wortwart/leaflet"
    },
    scripts: {
      start: "npm run dev",
    ...
    repository: {
      type: "git",
      url: "git://github.com/wortwart/leaflet-geoman.git"
    },
    author: {
      name: "Geoman.io",
  ...
  L_PM_Draw_default.Line = L_PM_Draw_default.extend({
    ...
    hasSelfIntersection() {
      const selfIntersection = turf_kinks_default(this._layer.toGeoJSON(15));
    ...
    _handleSelfIntersection(addVertex, latlng) {
      ...
      const selfIntersection = turf_kinks_default(clone2.toGeoJSON(15));
  ...
  function geojsonRbush(maxEntries) {
    ...
    tree.insert = function(feature3) {
      ...
      return import_rbush.default.prototype.insert.call(this, feature3);
    };
    ...
    tree.remove = function(feature3, equals) {
      if (feature3.type !== "Feature") throw new Error("invalid feature");
      feature3.bbox = feature3.bbox ? feature3.bbox : bbox(feature3);
      return import_rbush.default.prototype.remove.call(this, feature3, equals);
    };
    tree.clear = function() {
    ...
    flattenEach(
      ...
      function(line, _featureIndex, multiFeatureIndex) {
        ...
        for (let i = 0; i < coords.length - 1; i++) {
          ...
          start.properties.dist = distance(pt2, start, options);
          const startPos = getCoord(start);
          const stop = point(coords[i + 1]);
          stop.properties.dist = distance(pt2, stop, options);
          ...
          if (intersectPos) {
            intersectPt = point(intersectPos, {
              dist: distance(pt2, intersectPos, options),
              multiFeatureIndex,
  ...
  function nearestPointOnSegment(posA, posB, posC) {
    ...
    if (angle(A, I) > angleAB || angle(B2, I) > angleAB) {
      if (distance(vectorToLngLat(I), vectorToLngLat(A)) <= distance(vectorToLngLat(I), vectorToLngLat(B2))) {
    ...
    flattenEach(splitter2, function(point2) {
      ...
      if (!results.length) {
        ...
        results.forEach(function(feature3) {
          if (!feature3.bbox) feature3.bbox = square(bbox(feature3));
        });
  ...
  L_PM_Draw_default.Cut = L_PM_Draw_default.Polygon.extend({
    ...
    cut(layer) {
      ...
      const layers = Object.keys(all).map((l) => all[l]).filter((l) => l.pm).filter((l) => !l._pmTempLayer).filter(
        ...
        try {
          const lineInter = !!turf_line_intersect_default(layer.toGeoJSON(15), l.toGeoJSON(15)).features.length > 0;
    ...
    _cutLayer(layer, l) {
      ...
      if (l instanceof L.Polygon) {
        ...
        features.forEach((feature3) => {
          const lineDiff = turf_line_split_default(feature3, layer.toGeoJSON(15));
          let group;
          ...
          group.getLayers().forEach((lay) => {
            if (!turf_boolean_contains_default(layer.toGeoJSON(15), lay.toGeoJSON(15))) {
              lay.addTo(fg);
            }
          });
        });
  ...
  var RotateMixin = {
    ...
    _onRotate(e) {
      ...
      const angle2 = angleDiff + this._startAngle;
      this._setAngle(angle2);
      this._rotationLayer.pm._setAngle(angle2);
  ...
  L_PM_Edit_default.Line = L_PM_Edit_default.extend({
    ...
    _handleLayerStyle(flash) {
      ...
      if (this.options.allowSelfIntersection) {
        ...
        intersection3 = turf_kinks_default(this._layer.toGeoJSON(15));
    ...
    _checkMarkerAllowedToDrag(marker) {
      ...
      const prevLineIntersection = turf_line_intersect_default(
      ...
      const nextLineIntersection = turf_line_intersect_default(
        this._layer.toGeoJSON(15),
